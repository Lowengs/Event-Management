...
$type         = $_GET['type'] ?? $_REQUEST['type'] ?? 'pre';

if (!$assessmentId && $eventId) {
    // Find assessment by event + type
    $typeSearch = ($type === 'post') ? '%post%' : '%pre%';
    $stmt = $conn->prepare("SELECT assessment_id, title, type, instructions, time_limit FROM assessments WHERE event_id = ? AND LOWER(type) LIKE ? ORDER BY assessment_id DESC LIMIT 1");
    $stmt->bind_param("is", $eventId, $typeSearch);
    $stmt->execute();
...
try {
    ...
    $questions = [];
    if ($qResult) while ($q = $qResult->fetch_assoc()) $questions[] = $q;
    $qStmt->close();
