...
function updateVerificationBadges() {
  const badgeAS = document.getElementById('badgeAntiSpoof');
  const badgePC = document.getElementById('badgePresence');
  if (!currentEvent) return;

  if (badgeAS) {
    if (parseInt(currentEvent.AntiSpoofActive, 10) === 1) {
      badgeAS.className = 'badge-status warning';
      badgeAS.innerHTML = '<ion-icon name="scan-outline"></ion-icon> Anti-Spoof: Active';
    } else {
      badgeAS.className = 'badge-status neutral';
      badgeAS.innerHTML = '<ion-icon name="scan-outline"></ion-icon> Anti-Spoof: Idle';
    }
  }

  if (badgePC) {
    if (parseInt(currentEvent.PresenceCheckActive, 10) === 1) {
      badgePC.className = 'badge-status warning';
      badgePC.innerHTML = '<ion-icon name="pulse-outline"></ion-icon> Presence: Active';
    } else {
      badgePC.className = 'badge-status neutral';
      badgePC.innerHTML = '<ion-icon name="pulse-outline"></ion-icon> Presence: Idle';
    }
  }
}

async function triggerAntiSpoof() {
  ...
  try {
    ...
    const res = await fetch('../../config/API/endpoints/index.php?action=trigger_antispoofing', {
      method: 'POST',
      body: fd
    });
    const data = await res.json();
    if (data.success) {
      alert(data.message || 'Anti-spoofing challenge triggered for all attendees!');
      loadOnlineAttendance();
    } else {
      alert(data.message || 'Failed to trigger anti-spoofing.');
    }
  } catch (e) {
    console.error(e);
    alert('Network error triggering anti-spoofing.');
  }
}

async function triggerPresenceCheck() {
  const eventId = getSelectedEventId();
  if (!eventId) {
    alert('Please select an event first.');
    return;
  ...
  try {
    const fd = new FormData();
    fd.append('event_id', eventId);
    const res = await fetch('../../config/API/endpoints/index.php?action=trigger_presence_check', {
      method: 'POST',
      body: fd
    });
    const data = await res.json();
    if (data.success) {
      alert(data.message || 'Continuous presence check started for attendees!');
      loadOnlineAttendance();
    } else {
      alert(data.message || 'Failed to trigger presence check.');
    }
  } catch (e) {
    console.error(e);
    alert('Network error triggering presence check.');
  }
}

async function stopVerificationChecks() {
  const eventId = getSelectedEventId();
  if (!eventId) {
    alert('Please select an event first.');
    return;
  ...
  try {
    const fd = new FormData();
    fd.append('event_id', eventId);
    fd.append('check_type', 'all');
    const res = await fetch('../../config/API/endpoints/index.php?action=stop_verification_check', {
      method: 'POST',
      body: fd
    });
    const data = await res.json();
    if (data.success) {
      alert(data.message || 'Active verification checks stopped.');
      loadOnlineAttendance();
    } else {
      alert(data.message || 'Failed to stop checks.');
    }
  } catch (e) {
    console.error(e);
    alert('Network error stopping verification checks.');
  }
}
