(function () {
  ...
  function getFilteredAndSortedEvents() {
    ...
    list.sort((a, b) => {
      const dateA = a.EventDateTime ? new Date(a.EventDateTime.replace(/-/g, '/')).getTime() : 0;
      const dateB = b.EventDateTime ? new Date(b.EventDateTime.replace(/-/g, '/')).getTime() : 0;
  ...
  function renderGrid(events) {
    ...
    grid.innerHTML = events.map(ev => {
      ...
      const poster = ev.EventPicture
        ? (ev.EventPicture.startsWith('http') ? ev.EventPicture : '../../' + ev.EventPicture.replace(/^\/+/, ''))
        : '';
      ...
      if (stLower === 'completed') {
        ...
        actionBtnHtml = `<button class="ev-prereg-btn" data-eventid="${evId}" data-event='${modalData}' onclick="openPreregModal(this)"><i class='bx bx-user-plus'></i> Pre-Register</button>`;
      } else {
  ...
  async function openPreregModal(btn) {
    ...
    try {
      ...
      console.warn('Could not parse data-event JSON', e);
    }

    const eventId = Number(btn.dataset.eventid || (ev && ev.id) || (btn.closest('.event-card') ? btn.closest('.event-card').dataset.eventid : 0));
    if (!eventId) {
      console.error('No valid EventId found');
      return;
    }

    const card = btn.closest('.event-card');
    const eventName = (ev && ev.name) ? ev.name : (card ? card.querySelector('.ev-card-title')?.textContent?.trim() : 'this event');
    ...
    try {
      ...
      const res = await fetch('../../config/API/endpoints/index.php?action=event_register', {
        method: 'POST',
        body: fd
      });
      ...
      if (json && json.success) {
        ...
        if (item) {
          ...
          item.reg_count = Number(item.reg_count || item.registered_count || item.RegisteredCount || 0) + 1;
        }

        if (card) {
          const spotsEl = card.querySelector('.ev-spots');
          const capLabel = card.querySelector('.ev-cap-label');
          const capFill = card.querySelector('.ev-cap-fill');
          if (item) {
            const max = Number(item.EventCapacity || 0) > 0 ? Number(item.EventCapacity) : 100;
            const registered = Number(item.reg_count || 0);
            const remaining = Math.max(0, max - registered);
            const percent = Math.min(100, Math.round((registered / max) * 100));
            if (spotsEl) spotsEl.textContent = `${remaining} spot${remaining !== 1 ? 's' : ''} remaining`;
            if (capLabel) capLabel.textContent = `${registered}/${max}`;
            if (capFill) {
              capFill.style.width = `${percent}%`;
              if (percent >= 80) capFill.className = 'ev-cap-fill danger';
              else if (percent >= 50) capFill.className = 'ev-cap-fill warn';
            }
          }
        }

        const successMsg = `Successfully pre-registered for "${eventName}"!`;
        if (window.showAlertModal) {
          window.showAlertModal(successMsg, 'Pre-Registration Complete', 'success');
        } else if (typeof showModal === 'function') {
          showModal(successMsg, 'success', 'Pre-Registration Complete');
        } else {
          alert(successMsg);
        }
      } else {
        const msg = (json && json.message) ? json.message : 'Pre-registration failed.';
        if (json && json.already_registered) {
          registeredIds.add(eventId);
          btn.className = 'ev-prereg-btn ev-prereg-registered';
          btn.innerHTML = `<i class='bx bx-check-circle'></i> Registered`;
          btn.disabled = true;
      ...
      btn.disabled = false;
      btn.innerHTML = oldHtml;
      const errorMsg = 'Failed to pre-register due to a network connection error. Please try again.';
      if (window.showAlertModal) {
        window.showAlertModal(errorMsg, 'Connection Error', 'error');
      } else if (typeof showModal === 'function') {
        showModal(errorMsg, 'error', 'Connection Error');
      } else {
        alert(errorMsg);
      }
    }
  ...
  // Expose to window so onclick="openPreregModal(this)" immediately functions
